...
import { disableEditModal, removeFromChosenLabels } from "./notesSlice";
export function EditNote() {
    ...
    return (
        editNoteModalStatus && noteData && <div onClick={() => { dispatch(disableEditModal()) }} className="  fixed top-0 left-0 bottom-0 right-0  bg-dark-1 bg-opacity-50 flex items-center justify-center min-h-screen min-w-screen ">
            ...
            {status === "pending" && <p className="text-palette-yellow font-bold">Please wait while we save your note...</p>} */}
                ...
                <form ref={formRef} className="flex flex-col p-2 outline-none   " onSubmit={(e) => { e.stopPropagation(); submitNote(e) }}>
                    ...
                    <div className="flex flex-wrap">
                        {noteData.labels?.map(chosenLabel => <div key={chosenLabel?._id} className="text-sm p-1 pt-1 flex rounded-lg m-1 bg-selected-navitem-light dark:bg-selected-navitem-dark">
